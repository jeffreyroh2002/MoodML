...
output_dir = "results/106-radar-vocal-without-voiceless"  # Directory to save individual radar chart images
...
for f in Sorted_Song_list:
    ...
    voiceless_duration = song_radar_values[0]
    radar_values_without_voiceless = [val for i, val in enumerate(song_radar_values) if i != 0]
    avg_radar_values = radar_values_without_voiceless / (segment_count - voiceless_duration)

    # make new label_list without voiceless label
    new_label_list = []
    for label in label_list.values():
        if label != 'voiceless_split':
            new_label_list.append(label)

    # Output the averaged radar values to the terminal
    print(f"{f} Average Radar Values:")
    for label, avg_value in zip(new_label_list, avg_radar_values):
        print(f"{label}: {avg_value:.2f}")
    
    radar_chart_trace = go.Scatterpolar(
            r=avg_radar_values,
            theta=new_label_list,
            fill='toself',
            name=f"{f}_Average"
    )
