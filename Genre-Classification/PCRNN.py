...
DATA_PATH = "genre_dataset/genre_7x100_final_with_100_songs.json"
SAVE_MODEL = True
SAVE_HM = True

#OUTPUT DIR/FILE NAMES
NEWDIR_PATH = "results/0109_PCRNN_Genre7_final_100each_100ep_0.00001lr"
...
def get_heatmap(model, X_test, y_test, newdir_path, hm_name, label_list):
    ...
    labels = label_list  # Sort the labels
...
if __name__ == "__main__":
    ...
    # Train the model
    history = model.fit(X_train, y_train, validation_data=(X_validation, y_validation),
                        batch_size=32, epochs=EPOCHS, verbose=1, callbacks=[checkpoint], class_weight=class_weight_dict)
